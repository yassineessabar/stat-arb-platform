...
class StatArbStrategy:
    ...
    def check_entry_signals(self):
        ...
        try:
            ...
            signal_msg = f"SIGNAL CHECK - Current z-score: {z_score:.2f} | Prices: {self.config.symbol_1}={price1:.2f}, {self.config.symbol_2}={price2:.2f}"
            criteria_msg = f"ENTRY CRITERIA - Z-score threshold: {self.config.entry_z_score}, Current positions: {len(self.positions)}/{self.config.max_positions}"

            logger.info(signal_msg)
            logger.info(criteria_msg)

            # Also save to database
            if self.deployment_id and self.db:
                self.db.log_system_event(self.deployment_id, 'info', 'signal_analysis', signal_msg)
                self.db.log_system_event(self.deployment_id, 'info', 'signal_analysis', criteria_msg)
            ...
            if abs(z_score) > self.config.entry_z_score and len(self.positions) < self.config.max_positions:
                if z_score > self.config.entry_z_score:
                    ...
                    entry_msg = f"✅ ENTRY SIGNAL TRIGGERED - Z-score {z_score:.2f} > {self.config.entry_z_score} - Going SHORT"
                    logger.info(entry_msg)
                    if self.deployment_id and self.db:
                        self.db.log_system_event(self.deployment_id, 'info', 'signal_entry', entry_msg)
                    ...
                    entry_msg = f"✅ ENTRY SIGNAL TRIGGERED - Z-score {z_score:.2f} < -{self.config.entry_z_score} - Going LONG"
                    logger.info(entry_msg)
                    if self.deployment_id and self.db:
                        self.db.log_system_event(self.deployment_id, 'info', 'signal_entry', entry_msg)
                ...
                if abs(z_score) <= self.config.entry_z_score:
                    no_entry_msg = f"❌ NO ENTRY - Z-score {z_score:.2f} below threshold {self.config.entry_z_score}"
                    logger.info(no_entry_msg)
                    if self.deployment_id and self.db:
                        self.db.log_system_event(self.deployment_id, 'info', 'signal_entry', no_entry_msg)
                if len(self.positions) >= self.config.max_positions:
                    max_pos_msg = f"❌ NO ENTRY - Max positions reached {len(self.positions)}/{self.config.max_positions}"
                    logger.info(max_pos_msg)
                    if self.deployment_id and self.db:
                        self.db.log_system_event(self.deployment_id, 'warning', 'signal_entry', max_pos_msg)
    ...
    def check_exit_signals(self):
        ...
        try:
            if not self.positions:
                exit_msg = "EXIT CHECK - No open positions to check"
                logger.info(exit_msg)
                if self.deployment_id and self.db:
                    self.db.log_system_event(self.deployment_id, 'info', 'signal_exit', exit_msg)
                return

            check_msg = f"EXIT CHECK - Checking {len(self.positions)} open position(s)"
            logger.info(check_msg)
            if self.deployment_id and self.db:
                self.db.log_system_event(self.deployment_id, 'info', 'signal_exit', check_msg)
            ...
            exit_analysis_msg = f"EXIT CHECK - Current z-score: {z_score:.2f} | Exit threshold: {self.config.exit_z_score} | Stop loss: {self.config.stop_loss_z_score}"
            logger.info(exit_analysis_msg)
            if self.deployment_id and self.db:
                self.db.log_system_event(self.deployment_id, 'info', 'signal_exit', exit_analysis_msg)
    ...
    def _enter_position(self, direction: str, price1: float, price2: float, z_score: float):
        ...
        try:
            ...
            # Place actual orders for both paper trading (on testnet) and live trading
            orders = []
            try:
                if direction == 'SHORT':
                    # For SHORT: Sell symbol1, Buy symbol2
                    order1 = self._place_market_order(self.config.symbol_1, 'SELL', self.config.position_size)
                    order2 = self._place_market_order(self.config.symbol_2, 'BUY', self.config.position_size)
                else:  # LONG
                    # For LONG: Buy symbol1, Sell symbol2
                    order1 = self._place_market_order(self.config.symbol_1, 'BUY', self.config.position_size)
                    order2 = self._place_market_order(self.config.symbol_2, 'SELL', self.config.position_size)

                orders = [order1, order2]
                logger.info(f"📋 Placed {len(orders)} orders for {direction} position on {'Testnet' if self.config.testnet else 'Live'}")
                    ...
                    for order in orders:
                        if order:
                            self.db.log_system_event(
                                ...
                                'info',
                                'trade',
                                f"Order placed: {order['symbol']} {order['side']} {order['origQty']} @ {order.get('price', 'market')}"
                            )

            except Exception as e:
                logger.error(f"Failed to place orders: {e}")
                return  # Don't record position if orders failed
    ...
    def _place_market_order(self, symbol: str, side: str, quantity: float):
        ...
        try:
            # For Futures, we need to specify the quantity in USDT value
            # Convert quantity to proper format for futures trading
            params = {
                'symbol': symbol,
                'side': side,
                'type': 'MARKET',
            ...
            order = self.client._make_request('/fapi/v1/order', params, signed=True, method='POST')
            logger.info(f"✅ Order placed: {symbol} {side} {quantity} USDT - Order ID: {order.get('orderId')}")
            return order
